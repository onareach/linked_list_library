...
Word *createWordNode(char *s);

void insert_word_at_head(Word **link, Word *newWordNode);
...
void print_two_lists(Node *head1, Node *head2); // NEW
...
void delete_duplicates(Node *head); // NEW
Node *insert_after(Node *head, int new_value, int after_value); // NEW
Node *delete_list(Node *node); // NEW
...
// Two List Functions:
void add_lists(Node *head1, Node *head2); //NEW
void add_lists_shorter_recursive_version(Node *list1, Node *list2); // NEW
Node *duplicate_list(Node *switch_head); // NEW
Node *duplicate_list_shorter_version_recursive(Node *node); // NEW
Node *test_two_parameter_one_return(Node *node1, Node *node2); // NEW
Node *merge_sorted_lists(Node *list1, Node *list2); //NEW
...
int main(void)
{
    ...
    n = createWordNode("recognition");
    // First node at start of list - head is updated.
    insert_word_at_head(&head, n);
    // First node is also the tail.
    tail = n;

    n = createWordNode("realization");
    // Insert node at start of list - head is updated.
    insert_word_at_head(&head, n);

    n = createWordNode("revelation");
    // Insert node at end of list.
    insert_word_at_head(&tail->next, n);
    // Update tail.
    tail = n;

    n = createWordNode("conversation");
    insert_word_at_head(&head, n);
...
Word *createWordNode(char *s)
{
    Word *newWordNode = (Word *)malloc(sizeof(Word));
    newWordNode->s = s;
